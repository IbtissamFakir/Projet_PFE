...
const NotesModulePage = () => {
  ...
  useEffect(() => {
    if (selectedBranche) {
      setLoading(true);
      axios.get(`http://localhost:8000/api/stagiaires-par-branche/${selectedBranche}`)
        .then(res => {
  ...
  const handleInputChange = (stgId, code, val) => {
    if (val === "" || (parseFloat(val) >= 0 && parseFloat(val) <= 40)) {
      setSaisies(prev => ({ ...prev, [`${stgId}-${code}`]: val }));
    }
  };
  ...
  const handleValider = async () => {
    const data = Object.keys(saisies)
      ...
      .map(key => {
        ...
        return { stagiaire_id: id, type_code: code, note: saisies[key] };
      });

    try {
      await axios.post('http://localhost:8000/api/notes/bulk-store', { notes: data });
    ...
    <div className="p-8 bg-gray-50 min-h-screen font-sans text-gray-800">
      <div className="mb-10">
        <h1 className="text-3xl font-black uppercase tracking-tight">
          Saisie des notes : <span className="text-blue-600 italic">{nomModule}</span>
        </h1>
      ...
      <div className="mb-8 flex items-center gap-4">
        ...
        >
          ...
          {branches.map(br => <option key={br.id} value={br.id}>{br.intitule}</option>)}
        </select>
      ...
      <div className="bg-white rounded-[2.5rem] shadow-xl border border-gray-100 overflow-hidden">
        <table className="w-full text-left">
          <thead className="bg-gray-50/50 border-b border-gray-100">
            <tr>
              <th className="p-6 text-xs font-black uppercase tracking-widest text-gray-400" rowSpan="2">Stagiaire</th>
              <th className="p-4 text-center bg-blue-50/30 text-blue-700 font-black text-sm uppercase" colSpan="2">Semestre 1</th>
              <th className="p-4 text-center bg-purple-50/30 text-purple-700 font-black text-sm uppercase" colSpan="2">Semestre 2</th>
            </tr>
            <tr className="text-[11px] font-black uppercase text-center bg-white border-b border-gray-100">
              <th className="p-3 text-blue-500">CC1</th><th className="p-3 text-pink-500">CC2</th>
              <th className="p-3 text-blue-500">CC3</th><th className="p-3 text-pink-500">CC4</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-50">
            {stagiaires.length > 0 ? (
              stagiaires.map(stg => (
                <tr key={stg.id} className="hover:bg-blue-50/20 transition-colors">
                  <td className="p-6 font-bold text-gray-700">{stg.nomComplet}</td>
              ...
              <tr>
                <td colSpan="5" className="p-24 text-center text-gray-300 font-bold italic">
      ...
      <div className="mt-12 flex justify-end">
        <button 
          ...
          className="flex items-center gap-3 px-16 py-5 rounded-2xl font-black text-white bg-gradient-to-r from-emerald-400 to-teal-500 shadow-lg hover:shadow-emerald-200/50 disabled:opacity-30 transition-all active:scale-95"
        >
