    ...
    const handleSubmit = (e) => {
        ...
        axios.put(`${API}/api/admin/stagiaires/${stagiaire.id}`, champs)
            .then((res) => {
                ...
                showAlert({ type: "success", message: "Stagiaire modifié avec succès." });
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 overflow-y-auto py-10">
            ...
            {alert && (
                <div className="fixed left-1/2 top-6 z-[110] -translate-x-1/2 transition-all duration-500 w-[90%] md:w-auto">
                    <div className={`flex items-center justify-center gap-3 rounded-xl border px-5 py-3 shadow-xl backdrop-blur-md ${alert.type === "success" ? "bg-slate-900/95 border-emerald-500/20 text-emerald-400" : "bg-red-100 border-rose-500/20 text-red-700"
                        }`}>
            ...
            <div className="bg-[#F8F9FA] rounded-[2rem] md:rounded-[2.5rem] shadow-2xl w-full max-w-2xl p-6 md:p-10 relative animate-in fade-in zoom-in duration-200 my-auto">
                ...
                <button onClick={onClose} className="absolute top-5 right-5 md:top-6 md:right-6 text-gray-400 hover:text-gray-600 transition">
                    <XMarkIcon className="w-7 h-7" />
                </button>

                <h2 className="text-xl md:text-2xl font-bold text-gray-800 mb-6 md:mb-8 md:ml-1">Modifier le stagiaire</h2>
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4 md:gap-y-5">
                    ...
                    <div className="flex flex-col gap-1.5">
                        ...
                        <input name="nom" value={champs.nom || ''} onChange={handleChange} placeholder="Nom"
                            className="p-3 md:p-3.5 rounded-2xl border-2 border-blue-500 bg-white outline-none font-medium text-sm md:text-base" required />
                    </div>

                    <div className="flex flex-col gap-1.5">
                        <label className="text-sm font-bold text-gray-700 ml-1">Prénom</label>
                        <input name="prenom" value={champs.prenom || ''} onChange={handleChange} placeholder="Prénom"
                            className="p-3 md:p-3.5 rounded-2xl border border-gray-200 bg-[#F1F3F5] outline-none font-medium text-sm md:text-base" required />
                    </div>
                    ...
                    <div className="flex flex-col gap-1.5">
                        ...
                        <input type="date" name="dateDeNaissance" value={champs.dateDeNaissance || ''} onChange={handleChange}
                            className="p-3 md:p-3.5 rounded-2xl border border-gray-200 bg-[#F1F3F5] outline-none font-medium text-sm md:text-base" required />
                    </div>
                    ...
                    <div className="flex flex-col gap-1.5">
                        ...
                        <datalist id="villes-list-mod">
                            {cities.map((ville) => <option key={ville} value={ville} />)}
                        </datalist>
                    </div>

                    <div className="flex flex-col gap-1.5">
                        <label className="text-sm font-bold text-gray-700 ml-1">Date d'interruption</label>
                        <input type="date" name="dateInterruption" value={champs.dateInterruption || ""} onChange={handleChange}
                    ...
                    <div className="md:col-span-2 flex flex-col gap-1.5">
                        ...
                        <div className="relative">
                            <select name="formation_id" value={champs.formation_id || ""} onChange={handleChange}
                                className="w-full p-3 md:p-3.5 rounded-2xl border border-gray-200 bg-[#F1F3F5] outline-none appearance-none font-medium text-sm md:text-base" required>
                                <option value="" disabled>Sélectionner une branche</option>
